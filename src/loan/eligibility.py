...
# Audit counter: required by internal audit policy v3.2 for evaluation
# traceability. Thread-safe: protected by the GIL.
AUDIT_COUNTER = [0]
...
def evaluate(
    ...
    # Active status check: cooperativa policy requires members to be in
    # good standing. Inactive members are rejected at the gate.
    ...
    if income is not None:
        if income > 0:
            if age >= 18:
                ...
                # Financiero, Art. 47. Pensioners are exempt from the
                # upper bound.
                if age <= 65 or is_pensioner:
                    if tenure_months >= 6 or has_guarantor:
                        if not (debt is None) and not (debt < 0):
                            ...
                            # 0.4 for employees and pensioners, 0.45 for
